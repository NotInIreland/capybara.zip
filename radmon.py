            ...
            while(line):
                ...
                if len(params) == 2:
                    ...
                    if parameter == "user":
                        ...
                        elif value == "ludlum177":
                            self.protocol = self.LUDLUM177
...
class baseGeigerCommunication(threading.Thread):
    ...
    def sendCommand(self, command):
        ...
        while(self.serialPort.inWaiting() > 0 and self.stopwork == 0):
            response = response + self.serialPort.read().decode('windows-1252')
        return response
    ...
    def getResult(self):
        ...
        if len(self.queue) > 0:
            ...
            data = [-1, datetime.datetime.now(datetime.timezone.utc)]
...
class ludlum177(baseGeigerCommunication):
        ...
        try:
            ...
            for i in range(0, 10):
                if doneSend is False:
                    if data is not None:
                        httpResponse = str(data.decode('utf-8')).splitlines()[0]
                        print ("Server response => geiger 1: ", httpResponse, "\r\n")
                        logger.info("Server response => geiger 1: " + httpResponse)
                        if "incorrect login" in data.decode('utf-8').lower():
...
def main():
    ...
    if (os.path.isfile("config.txt") == 0):
        ...
        try:
            ...
            f.write("speed=2400\r\n")
            f.write("# Protocols: demo, mygeiger, gmc, netio, ludlum177, audio\r\n")
        ...
        elif cfg.protocol == config.LUDLUM177:
            print ("Using Ludlum 177 protocol => geiger 1\r\n")
            logger.info("Using Ludlum 177 protocol => geiger 1")
            geigerCommunication = ludlum177(cfg)
        else:
        ...
        for numThread in threading.enumerate():
            if numThread.daemon: continue
            if numThread.name == 'MainThread': continue
            print ("Stopping alive thread: ", numThread.name, "\r\n\t")
            logger.info("Stopping alive thread: " + numThread.name)
            numThread.stop()
            time.sleep(1)
